        ...
        for t in itertools.count():
            ...
            if tau >= 0:
                ...
                for i in range(tau+1, min(tau+n, T)+1):
                    G += discount_factor**(i-tau-1) * reward_memory[i]
                
                # (3) If tau + n >= T, G_{t:t+n} = G_{t}
                if tau + n < T:
                    G += discount_factor**n * Q[state_memory[tau+n]][action_memory[tau+n]]
                
                # (4) TD update
                td_delta = G - Q[state_memory[tau]][action_memory[tau]]
                Q[state_memory[tau]][action_memory[tau]] += alpha* td_delta
            ##################################################################### 
            if tau == T-1 :
                break
            if t > 50:
